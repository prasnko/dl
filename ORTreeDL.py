...
##    partialSetTest()
    ...
    global maxLevel
    maxLevel = 0
    f = open('output.txt-PGM','w')
##    train = open(r'C:\Prasanna\Spring13\ML\HW1\data\train.txt')
##    test = open(r'C:\Prasanna\Spring13\ML\HW1\data\test.txt')
##    train = open(sys.argv[1])
##    test = open(sys.argv[2])
    train = open(r'C:\Prasanna\Spring13\PGM\HW5\hw5-data\1.uai.data\train-f-1.txt')
    test = open(r'C:\Prasanna\Spring13\PGM\HW5\hw5-data\1.uai.data\test.txt')
    ...
    graph.write_png('ProbabilisticTree-PGM.png')
...
##        tree,outputTree=removeIndependentNodes(tree,outputTree,0)
        ...
        graph = pydot.Dot(graph_type='graph')
        visit(graph,tree,0,0)
        graph.write_png('PrunedProbabilisticTree-ind-PGM'+str(i)+'.png')
        outputTree = dict(tree)
        tree = visitPruneUniform(tree,outputTree,0)
        graph = pydot.Dot(graph_type='graph')
        visit(graph,tree,0,0)
        graph.write_png('PrunedProbabilisticTree-uni-PGM'+str(i)+'.png')
        print '\n'
        print tree
