...
match = [0,0]
removeUniform = [0,0]
def visitPruneUniform(node,outputNode):
    global counts,match,removeUniform
    for k,v in node.iteritems():
     if isinstance(v, dict):
        # We start with the root node whose parent is None
        # we don't want to graph the None node
        visitPruneUniform(node[k],outputNode[k])
        if removeUniform[0]>0:
...
##        tree,outputTree=removeIndependentNodes(tree,outputTree,0)
        ...
        outputTree = dict(tree)
        tree = visitPruneUniform(tree,outputTree)
        graph = pydot.Dot(graph_type='graph')
        visit(graph,tree,0,0)
